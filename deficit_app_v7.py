...
def run_sf_report(instance_url: str, session_id: str, report_id: str,
    ...
    if date_col_id:
        today    = _date.today()
        existing = rpt_meta.get("reportFilters") or []
        all_dfs  = []
        failed   = False
        ...
        for i in range(9):
            ...
            body = {
                "reportMetadata": {
                    "showDetails": True,           # required for Summary reports to return detail rows
                    "reportFilters": existing + [
    ...
    r = requests.post(f"{api}/instances", headers=h,
                      json={"reportMetadata": {"showDetails": True}}, timeout=30)
    r.raise_for_status()
    inst_id = r.json()["id"]
    for _ in range(90):
        time.sleep(2)
