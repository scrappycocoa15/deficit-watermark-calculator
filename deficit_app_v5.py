...
def run_sf_report(instance_url: str, session_id: str, report_id: str):
    """
    Fetch all report rows.  Strategy:
      1. GET report metadata → discover the Salesforce object type and each
         column's SOQL field name (entityColumnName).
      2. If every column maps to a field name, execute a paginated SOQL query
         against /query — no row limit; follows nextRecordsUrl automatically.
      3. If any column lacks an entityColumnName (e.g. a formula field the
         Analytics API synthesises), fall back to the async Analytics API
         instance endpoint (hard-capped at 2,000 rows, warns the caller).
    Returns (DataFrame, all_data: bool).
      all_data=True  → all rows were fetched (SOQL path).
      all_data=False → hit the 2,000-row Analytics API ceiling.
    """
    h   = _hdr(session_id)
    ...
    # ── Step 2: SOQL path (unlimited rows) ───────────────────────────────────
    if obj_type and all(ent_cols):
        try:
            ...
            while True:
                if next_url:
                    resp = requests.get(
                        f"{instance_url}{next_url}", headers=h, timeout=30
                    )
                else:
                    resp = requests.get(
                        f"{instance_url}/services/data/{SF_API_VERSION}/query",
                ...
                for rec in data.get("records", []):
                    ...
                    for field in ent_cols:
                        # Resolve cross-object dot-notation: "Account.Name"
                        val = rec
            ...
            pass  # fall through to async Analytics API

    # ── Step 3: async Analytics API fallback (2,000-row cap) ─────────────────
    r = requests.post(f"{api}/instances", headers=h, timeout=30)
    r.raise_for_status()
    inst_id = r.json()["id"]
    ...
    try:
        with st.spinner("Running Sales report…"):
            s_df, s_all = run_sf_report(
                st.session_state["sf_instance"], st.session_state["sf_token"], SALES_REPORT_ID,
            )
        if not s_all:
            warns.append(
                "Sales report fell back to the Analytics API (one or more columns "
                "have no SOQL field mapping). Results are capped at 2,000 rows and "
                "may be incomplete."
            )
        ...
        with st.spinner("Running Terminations report…"):
            t_df, t_all = run_sf_report(
                st.session_state["sf_instance"], st.session_state["sf_token"], TERMS_REPORT_ID,
            )
        if not t_all:
            warns.append(
                "Terminations report fell back to the Analytics API (one or more columns "
                "have no SOQL field mapping). Results are capped at 2,000 rows and "
                "may be incomplete."
            )
