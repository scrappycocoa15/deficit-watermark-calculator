...
def run_sf_report(instance_url: str, session_id: str, report_id: str):
    """Execute a Salesforce report asynchronously. Cached for 5 minutes."""
    h   = _hdr(session_id)
    api = f"{instance_url}/services/data/{SF_API_VERSION}/analytics/reports/{report_id}"

    # Kick off async instance
    r = requests.post(f"{api}/instances", headers=h, json={}, timeout=30)
    r.raise_for_status()
    inst_id = r.json()["id"]
...
with dl1:
    st.download_button(
        ...
        data=excel_bytes(results, cses_df=cses_df),
