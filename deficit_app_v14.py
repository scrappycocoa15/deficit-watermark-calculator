...
def run_sf_report(instance_url: str, session_id: str, report_id: str,
    ...
    if date_col_id:
        ...
        # Re-send the report's saved filters alongside the new date-range filter.
        # Strip each saved filter to ONLY {column, operator, value} — the GET
        # response includes extra fields (e.g. isRunPageOnly, filterType) that
        # Salesforce rejects in a POST body.  Also skip run-page-only filters;
        # they are UI-only and don't belong in an API call.
        saved_filters = [
            {"column": f["column"], "operator": f["operator"], "value": str(f["value"])}
            for f in (rpt_meta.get("reportFilters") or [])
            if f.get("column") and f.get("operator")
            and not f.get("isRunPageOnly", False)
        ]
        ...
        for i in range(9):
            ...
            for _ in range(60):          # max 60 s per chunk
                ...
                if status in ("Error", "Cancelled"):
                    chunk_errors += 1
                    break   # skip this chunk; continue to next month
        ...
        # Only keep the Tier-2 result if we actually collected rows.
        # If every chunk silently errored or was empty, fall through to Tier 3
        # rather than returning a zero-row DataFrame with all_data=True.
        if not failed and all_dfs:
            return pd.concat(all_dfs, ignore_index=True), True

    # ── Tier 3: plain async Analytics API (2,000-row cap) ────────────────────
    # Always send a JSON body so Salesforce sees Content-Type: application/json.
    ...
    try:
        ...
        if not s_all:
            warns.append(
                ...
                "Monthly chunking produced no rows — the date-column filter may not be supported "
                f"for this report type. Row count shown is capped at 2,000 if your report exceeds that."
            )
...
if st.session_state.get("diag"):
    ...
    with st.expander("Diagnostics — expand if results look wrong", expanded=(d["active_total"] == 0)):
        ...
        tier_s = "Chunked (Tier 2)" if d.get("sales_all") else "Plain async / Tier 3 (≤2,000 rows)"
        tier_t = "Chunked (Tier 2)" if d.get("terms_all") else "Plain async / Tier 3 (≤2,000 rows)"
