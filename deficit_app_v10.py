    ...
    for row in all_rows:
        ...
        for i, cell in enumerate(row.get("dataCells", [])):
            ...
            if dtype in numeric_t:
                val = cell.get("value")
                # Salesforce sometimes returns value=null even for numeric cells
                # (e.g. converted-currency fields). Fall back to parsing the label.
                if val is None:
...
def run_sf_report(instance_url: str, session_id: str, report_id: str,
    ...
    if obj_type and all(ent_cols):
        try:
            ...
            return pd.DataFrame(all_rows, columns=col_labels), True
        except Exception:
            pass  # fall through
    ...
    if date_col_id:
        ...
        # 9 monthly windows — comfortably covers any 6-month deficit window.
        # We send ONLY the new date-range filters in the POST body.
        # Do NOT re-send the existing saved filters: the GET response includes
        # extra fields (e.g. isRunPageOnly) that Salesforce rejects in a POST,
        # and the report's own saved filters are preserved automatically when
        # reportFilters is omitted from all other metadata keys.
        for i in range(9):
            ...
            chunk_meta: dict = {
                "reportFilters": [
                    {"column": date_col_id, "operator": "greaterOrEqual", "value": start},
                    {"column": date_col_id, "operator": "lessOrEqual",    "value": end},
                ]
            }
            # showDetails only valid for Summary / Matrix — omit for Tabular
            if needs_detail:
                chunk_meta["showDetails"] = True

            body = {"reportMetadata": chunk_meta}
            ...
            chunk_df = None
            for _ in range(60):          # max 60 s per chunk
                time.sleep(1)
                r2 = requests.get(f"{api}/instances/{inst_id}", headers=h, timeout=30)
                r2.raise_for_status()
                payload = r2.json()
                ...
                if status == "Success":
                    ...
                    if not chunk_all:
                        # A single month exceeded 2,000 rows — chunking won't work
                        failed = True
                    break
                if status in ("Error", "Cancelled"):
                    break   # skip empty/errored chunk; continue to next month
        ...
        if not failed:
            combined = pd.concat(all_dfs, ignore_index=True) if all_dfs \
                       else pd.DataFrame(columns=col_labels)
            return combined, True

    # ── Tier 3: plain async Analytics API (2,000-row cap) ────────────────────
    t3_body = {"reportMetadata": {"showDetails": True}} if needs_detail else None
...
def _coerce_numeric(s: pd.Series) -> pd.Series:
    """
    Convert a Series to numeric, handling Salesforce accounting formats:
      "$1,234.56"   →  1234.56
      "(1,234.56)"  →  -1234.56   ← accounting notation; previously parsed as NaN
      "($1,234.56)" →  -1234.56
    """
    if pd.api.types.is_numeric_dtype(s):
        return pd.to_numeric(s, errors="coerce")
    # map returns Python None for unparseable values; to_numeric converts None → NaN
    return pd.to_numeric(s.map(_parse_sf_number), errors="coerce")
...
if st.session_state.get("connected") and "run_btn" in dir() and run_btn:
    ...
    try:
        ...
        if not s_all:
            warns.append(
                "Sales report fell back to the 2,000-row Analytics API. "
                "Results may be incomplete. Check that the report's date column "
                f"label exactly matches \"{SALES_DATE_COL}\"."
            )
        ...
        if not t_all:
            warns.append(
                "Terminations report fell back to the 2,000-row Analytics API. "
                "Results may be incomplete. Check that the report's date column "
                f"label exactly matches \"{TERMS_DATE_COL}\"."
            )
