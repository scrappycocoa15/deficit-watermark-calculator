...
def run_sf_report(instance_url: str, session_id: str, report_id: str,
    ...
    if date_col_id:
        today   = _date.today()
        all_dfs = []
        t2_errs = []   # Salesforce error payloads — surfaced in diagnostics

        # Strip saved filters to {column, operator, value} only.
        saved_filters = [
        ...
        # POST directly to /reports/{id} (no /instances) so each monthly slice
        # runs synchronously and returns data — or a precise HTTP 400 body if
        # Salesforce rejects the filter, instead of a silent "Error" instance.
        # 9 monthly windows covers any 6-month deficit window with a buffer.
        for i in range(9):
            ...
            chunk_meta = {
                "reportFilters": saved_filters + [
                    # Single "between" filter replaces the previous two-filter
                    # (greaterOrEqual + lessOrEqual) approach that was being
                    # rejected by Salesforce instance processing.
                    {"column": date_col_id, "operator": "between",
                     "value": f"{start},{end}"},
                ]
            }
            ...
            if r.status_code >= 400:
                # Capture the exact Salesforce error for diagnostics
                try:
                    err_body = r.json()
                except Exception:
                    err_body = r.text[:800]
            ...
            if not chunk_all:
                # This single month exceeds 2,000 rows — monthly chunking won't
                # help.  Discard partial data and fall through to Tier 3.
                t2_errs.append({
                    "month":  f"{y}-{mo:02d}",
                    "status": "cap_exceeded",
                    "body":   "Single month > 2,000 rows; chunking cannot help.",
                })
                all_dfs = []
                break
        ...
        if all_dfs:
            return pd.concat(all_dfs, ignore_index=True), True, t2_errs

        # No data collected (all chunks errored or empty) — fall through.
        # t2_errs carries the exact Salesforce rejection reason.

    # ── Tier 3: plain async Analytics API (2,000-row cap) ────────────────────
    # Last resort.  Runs the report with its default configuration and no date
    # chunking.  Returns at most 2,000 rows; all_data=False signals truncation.
...
if st.session_state.get("connected") and "run_btn" in dir() and run_btn:
    ...
    try:
        with st.spinner("Running Sales report (monthly chunks)…"):
            s_df, s_all, s_t2errs = run_sf_report(
        ...
        with st.spinner("Running Terminations report (monthly chunks)…"):
            t_df, t_all, t_t2errs = run_sf_report(
                st.session_state["sf_instance"], st.session_state["sf_token"],
